...
##
class PDLPackConfigError(Exception):
...
def topsort(depdict):
    ...
    while len(depdict) > 0:
        ...
        if newdepdict == depdict:
            raise PDLPackConfigError(str(depdict))
        else:
            depdict = newdepdict
        if found > 0:
            curlevel += 1
...
# @param conf a pdlpack configuration
##
def topsort_modules(conf):
    ...
    except PDLPackConfigError as e:
        raise PDLPackConfigError("invalid cyclic dependency between modules: " + e.value + "; check Modules.yml files")
