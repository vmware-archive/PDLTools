...
##         ssh ${host} 'rm -rf ${DATA_DIR}/gpdb-data/primary* ${DATA_DIR}/gpdb-data/mirror*'
##     done
...
ENVIRONMENT_FILE=${ENVIRONMENT_FILE:=env-${GPDB_VERSION}-${GPDB_BUILD_VERSION}.sh}

cat > ${BASE_DIR}/${ENVIRONMENT_FILE} <<-EOF
	## ======================================================================
	## created: `date`
	## ======================================================================
...
if [ "${USE_GPPERFMON}" = "true" ]; then
	cat >> ${BASE_DIR}/${ENVIRONMENT_FILE} <<-EOF
		source ${GPPERFMON_INSTALL_DIR}/gpcc_path.sh
	EOF
fi
...
if [ "${SKIP_INSTALL}" != "true" ]; then
    install_gpdb
    tar zxvf ${BASE_DIR}/product/qautils-RHEL5-x86_64.tar.gz -C ${GPDB_INSTALL_DIR}
fi
...
for host in `cat ${HOSTS_CONF}`; do
    echo rsync -au --delete ${DATA_DIR}/gpdb ${host}:${DATA_DIR}/
    rsync -au --delete ${DATA_DIR}/gpdb ${host}:${DATA_DIR}/
    echo rsync -au --delete ${DATA_DIR}/greenplum-db ${host}:${DATA_DIR}/
    rsync -au --delete ${DATA_DIR}/greenplum-db ${host}:${DATA_DIR}/
done
...
if [ "${USE_GPPERFMON}" = "true" ]; then
    ...
    cat >> ${MASTER_DATA_DIRECTORY}/pg_hba.conf <<-EOF
		host all       gpmon 192.168.2.201/32  md5
...
##       python TEST.py > TEST.log 2>&1 & )
    ...
    if [ -z "${CMDR_INSTALLER_PROJECT}" ]; then
        CMDR_INSTANCE_NAME="GPCC QA ($( hostname -s ))"
    else
        CMDR_INSTANCE_NAME="GPCC QA (${CMDR_INSTALLER_PROJECT})"
    fi
    ...
    ( source ${BASE_DIR}/env-${GPDB_VERSION}-${GPDB_BUILD_VERSION}.sh; \
      ...
      gpcmdr --version; \
      echo ""; echo "======================================================================"; \
      cat ${BASE_DIR}/conf/gpperfmon-setup-input.txt | gpcmdr --setup )
